        ...
        <Modal title="Order Form" v-if="show_modal" classes="full-mask half-body" @close='closeModal'>
            <div class="form">
                <div class="form-input" v-for="(input, index) in product.order_forms" :key="index">
                    ...
                    <input :type="input.field_type" v-model='form[input.field_alias]' v-if="['one-choice option', 'multi-choice']">                  
                    <select v-if="input.field_type === 'one-choice option'">
                        <option v-for="(option, index) in JSON.parse(input.field_options)" :key="index">{{option}}</option>
                    </select>
                </div>
                <div class="form-cta">
            ...
            <div class="product__header">
                <div class="product__images">
                    ...
                    <div class="product__images__items" v-if="productImages && productImages.length > 0">
                        <div class="product__images__items__item" v-for="(image, index) in productImages" :key="index" >
                            <img :src="image" alt="">
                        </div>
                    </div>
                </div>
                ...
                <div class="product__details">
                    <div class="product__details__name">
                        ...
                        <p>{{product.name}}</p>
                        <p>by <span> {{product.business.name}}</span></p>
        ...
        <div class="tabbed-content">
            <div class="tabbed-content__container">
                <div class="tabbed-content__tab">
                    <div class="tab" @click="activeTab = tab" :class="[activeTab === tab ? 'selected' : '']" v-for="(tab, index) in tabs" :key="index">
                        <p>{{tab}}</p>
                    </div>
...
export default {
        ...
        computedPrice() {
            if (this.product) {
                if (this.qty === 1) {
                    ...
                    return parseFloat(parseFloat(this.product.unitprice) * this.qty).toFixed(2)
                }
            }
        },
        productImages() {
    ...
    methods: {
        ...
        createOrder() {
          ...
          this.$api.post(`/products/${this.product.id}/order`, payload).then(resp=> {
           ...
           //this.createOrder()
            FlutterwaveCheckout({
                ...
                customer: {
                    ...
                    name: this.form["Full Name"] ? this.form["Full Name"] : this.form.first_name + this.form.last_name
                },
                customizations: {
                    title: "Afflee",
                ...
                callback: (data) => {
                    ...
                    let payload = {type: 'purchase', referrer:this.$route.query.referrer, order_id: this.order.id, ...data}
        ...
        getProduct() {
            ...
            this.$api.get('/products/pub/' + slug).then(product=> {
