<template>
  <div>
    ...
    <div class="marketers">
      ...
      <FindMarketersModal
        classes="sixty-body"
        @close="closeFindMarketersModal"
        :show="actionFindMarketers"
        v-if="actionFindMarketers"
        :campaign="campaign"
      ...
      <div class="campaign__marketers">
        ...
        <Listing
          v-if="campaign_marketers.list && campaign_marketers.list.length > 0 && !loading.marketers"
          :records="campaign_marketers.list"
          :pagination="campaign_marketers.page_info"
          :labels="campaign_marketers.labels"
          :constants="computedConstants"
        ></Listing>
        <template v-else-if="loading.marketers">
          <div class="loading">Loading...</div>
        </template>
        <template v-else>
          <div class="center">
            <EmptyState
              caption="No marketers have joined your campaign"
              action="Find marketers"
              @onActionClicked="findMarketers"
            ></EmptyState>
          </div>
        </template>
      </div>
...
export default {
      ...
      this.$api
        ...
        .catch(err => {
          this.$store.dispatch("dashboard/actionShowErrorToast", {
            message: "Couldn't change the status of this campaign"
          });
        });
    },
    addLink() {
  ...
  computed: {
    ...
    computedConstants() {
      return {
        currency: (this.campaign && this.campaign.currency) || "NGN"
      };
    },
