<template>
  <div class="product">
    <EditProductDetailsForm
      :show="edit_product_details"
      :product="product"
      @close="closeEditDetails"
    />
    <EditDeliveryDetailsForm
      :show="edit_product_delivery_details"
      :details="productDeliveryDetails"
      :product="product"
      @close="toggleEditDeliveryDetails"
    />
    <EditProductFeaturesForm
      :show="edit_product_features"
      :features="productFeatures"
      :product="product"
      @close="toggleEditFeatures"
    />
    <EditVariationDetailsForm
      :show="edit_product_variations"
      :details="productVariationDetails"
      :product="product"
      @close="toggleEditVariationDetails"
    />
    <EditProductImagesForm
      :show="edit_product_images"
      :images="productImages"
      :product="product"
      @close="toggleEditProductImages"
    />

    <!--- share modal -->

    <ShareCampaignLinkModal
      :advert_note="product.advert_note"
      :link="marketer_link"
    ...
    <ProductTabs v-if="product && Object.keys(product).length > 0" :params="{id: product.id}" />
    <div>
      <div class="mobile-share" @click="show_share_modal=true">
        <div class="mobile-share__container">
          <div class="class">
            ...
            >
              <path
                d="M7.51002 2.23001L16.07 6.51001C19.91 8.43001 19.91 11.57 16.07 13.49L7.51002 17.77C1.75002 20.65 -0.59998 18.29 2.28002 12.54L3.15002 10.81C3.37002 10.37 3.37002 9.64001 3.15002 9.20001L2.28002 7.46001C-0.59998 1.71001 1.76002 -0.649988 7.51002 2.23001Z"
                stroke="white"
                stroke-width="1.5"
          ...
          <div class="page__header">
            <div class="page__header__content">
              <div class="page__header__status">
                <div
                  class="page__header__status__indicator green-fill"
        ...
        <div class="half">
          <div class="page">
            ...
            <div class="variation" v-if="productVariationDetails">
              <div
                class="variation__body"
                v-for="(detail, index) in productVariationDetails"
                :key="index"
              >
          ...
          <div class="page">
            ...
            <div class="page__content" v-if="productDeliveryDetails">
              <div
                ...
                v-for="(detail, index) in productDeliveryDetails"
                :key="index"
              >
                <p>{{detail.name}}</p>
                <p>{{detail.value}}</p>
...
export default {
    ...
    getMarketerLink() {
      if (this.product && this.product.id) {
        this.$api.get(`/links?product_id=${this.product.id}`).then(resp => {
          const link =
            resp.data.data.redir_links[0] &&
            resp.data.data.redir_links[0].redir_link;
  ...
  computed: {
    ...
    productVariationDetails() {
      return (
        this.product.variation_details &&
        JSON.parse(this.product.variation_details).length &&
        JSON.parse(this.product.variation_details)
      );
    },
    productDeliveryDetails() {
      return (
        this.product.delivery_details &&
        JSON.parse(this.product.delivery_details).length > 0 &&
        JSON.parse(this.product.delivery_details)
      );
    }
  }
};
