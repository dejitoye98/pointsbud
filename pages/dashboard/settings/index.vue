  ...
  <div class="settings">
      ...
      <Panel title="Team members" :show_execute_button="false">
        <div class="flex">
          <div class="card card--nopad bank-list">
            ...
            <div
              class="empty"
              v-else-if="!team_members.list || team_members.list.length < 1 && !loading_teams"
            >
            ...
            <div class="bank-item" v-for="member in team_members.list" :key="member.id">
              <div class="flex flex--between">
                <p class="bank-item__name">{{member.name}}</p>
                <!--<span class="remove remove--desktop" @click="deleteAccount(account.id)">Remove</span>-->
              </div>
              <p>Joined: {{formatDate(member.createdAt)}}</p>
              <!--<span class="remove remove--mobile" >Remove</span>-->
            </div>
            <div
              class="bank-item__invitations"
              v-if="team_members.invitations.length > 0"
            >Invitations</div>
            <div class="bank-item" v-for="member in team_members.invitations" :key="member.id">
              <div class="flex flex--between">
                <p class="bank-item__name">{{member.invited_email}}</p>
                <!--<span class="remove remove--desktop" @click="deleteAccount(account.id)">Remove</span> -->
              </div>
              <p>Not joined yet</p>
              <!--<span class="remove remove--mobile" >Remove</span>-->
      ...
      <Panel title="Bank Accounts" :show_execute_button="false">
        <div class="flex">
          <div class="card card--nopad bank-list">
            ...
            <div
              class="empty"
              v-else-if="!bank_accounts || bank_accounts.length < 1 && !loading_bank_accounts"
            >
              <!--No bank accounts added yet-->
            ...
            <div class="bank-item" v-for="account in bank_accounts" :key="account.id">
              <div class="flex flex--between">
                <p class="bank-item__name">{{account.bank_name}}</p>
                <span class="remove remove--desktop" @click="deleteAccount(account.id)">Remove</span>
              </div>
    ...
    <template v-if="activeTab === 'Password'">
      <Panel
        ...
        :disable_execute_button="!canChangePassword"
        @onExecute="changePassword"
      >
        <div class="password">
