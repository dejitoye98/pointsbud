<template>
  <div class="earnings">
    <div class="earnings__overvieww">
      <div class="currencies">
        <div
          ...
          :class="[earnings_currency === 'NGN' ? 'selected' : '']"
          @click="switchCurrency('NGN')"
        >NGN</div>
        <div
          class="currencies__item"
          :class="[earnings_currency === 'GHS' ? 'selected' : '']"
          @click="switchCurrency('GHS')"
        >GHS</div>
        <div
          class="currencies__item"
          :class="[earnings_currency === 'KES' ? 'selected' : '']"
          @click="switchCurrency('KES')"
        >KES</div>
        <div
          class="currencies__item"
          :class="[earnings_currency === 'USD' ? 'selected' : '']"
      ...
      <div class="panel" v-else>
        ...
        <div class="panel__item">
          <p>{{earnings_currency}} {{ wallet && wallet[0] ? formatMoney(wallet[0].witheld_balance) : 0}}</p>
          <p>Witheld Balance</p>
        </div>
        <div class="panel__item">
    ...
    <div class="section">
      ...
      <Listing
        item_link="/dashboard/transactions/:id"
        :show_pagination="true"
        :pagination="earnings_page_info"
        :records="earnings"
        :labels="earnings_history_labels"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getWallet() {
      this.loading = true;
      this.$api
        .get("/wallets?currency=" + this.earnings_currency)
        .then(resp => {
          this.loading = false;
    ...
    getEarningsStats() {
      //this.loading = true;
      this.$api
        .get("/earnings/stats?currency=" + this.earnings_currency)
        .then(resp => {
    ...
    getEarnings() {
      this.$api
        .get("/earnings?currency=" + this.earnings_currency)
        .then(resp => {
