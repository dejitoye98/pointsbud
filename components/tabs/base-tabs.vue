    ...
    <div class="settings">
        <div class="settings__tabs">
            <template v-if="userContext === 'business' && business_tabs">
                <div v-for="tab in business_tabs.filter(tab=> tab.show === true || typeof(tab.show) === 'undefined')" class="settings__tabs__tab" :class="[activeTab == tab.name ? 'active' : '']" :key="tab.name" @click="getRoutes(tab)">{{tab.name}}</div>
            </template>
            <template v-else-if="userContext === 'marketer' && marketer_tabs">
                <div v-for="tab in marketer_tabs.filter(tab=> tab.show === true || typeof(tab.show) === 'undefined')" class="settings__tabs__tab" :class="[activeTab == tab.name ? 'active' : '']" :key="tab.name" @click="getRoutes(tab)">{{tab.name}}</div>
            </template>
        </div>
    </div>
</template>
    ...
    created() {
        ...
        if (this.marketer_tabs) {
            this.marketer_tabs.forEach(tab=> {
                console.log('tab equality is: ')
                console.log(window.location.pathname === this.computeTabRoute(tab))
                if (window.location.pathname === this.computeTabRoute(tab)) {
                    this.activateTab(tab)
                }
            })
