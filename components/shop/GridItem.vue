<template>
    ...
    >
      <div class="thumbnail">
        <div class="overlay" v-if="product.availability === 'out-of-stock'"></div>
        <img
        ...
        <div class="quantity" @click.stop>
          <button
            ...
            v-if="!isInCart"
            :style="{
              backgroundColor: styling?.primary_color ? styling.primary_color : 'black',
              color: styling?.text_on_primary ? styling.text_on_primary : 'white'
            }"
          >
          ...
          <template v-else>
            <div
              style="display: flex; width: 100%; box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px;"
              :style="{
                backgroundColor: styling?.primary_color ? styling.primary_color : 'black'
              }"
            >
      ...
      <div style="margin: 5px 0; padding: 0 8px">
        <p class="name">{{ product.name }}</p>
  
        <div class="quantity-available" v-if="product.availability === 'available' && false">
          <div class="available-indicator" :class="product.quantity_available > 3 ? '' : 'available-indicator--few'"></div>
          <p :class="product.quantity_available > 3 ? 'many-available' : 'few-left'">
            {{ product.quantity_availablen > 3 ? 'Plenty available' : 'Few left' }}
          </p>
        </div>
        ...
        <template v-else>
          <div class="product-price">
            <p class="price golden-price">
              {{ 'NGN' | currencySymbol }}{{ product.unitprice | money }}
            </p>
            <p class="price strikethrough" v-if="product.original_price">
      ...
      <div style="justify-content: flex-end; padding: 6px 0" v-if="false">
        <button
          @click="onSelect"
          style="width: 100%; font-size: 16px; background-color: white; color: black; border: 1px solid black; font-weight: 500; border-radius: 20px; padding: 8px 16px"
        >
          Select
        </button>
      </div>
  ...
  import LiveMenuModal from "../modals/LiveMenuModal.vue";
  ...
  export default {
    ...
    components: { TruncatedText, LiveMenuModal }
  };
  </script>
