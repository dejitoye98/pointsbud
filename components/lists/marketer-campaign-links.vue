    ...
    <template v-if="data && data.length > 0">
      <table>
        ...
        <tr v-for="(data, index) in data" :key="index">
          ...
          <td
            data-th="Pay Per Click"
          >{{data.pay_currency || "NGN"}} {{data.pay_per_click && data.pay_per_click}}</td>
          <td
            data-th="Pay Per Sale"
          >{{data.pay_currency || "NGN"}} {{data.pay_per_sale && data.pay_per_sale}}</td>
...
export default {
  ...
  methods: {
    triggerShare(data) {
      ...
      this.share.advert_note = data.product
        ? data.product.advert_note
        : data.campaign_link.advert_note || this.campaign.advert_note;
    ...
    getCampaignLinks() {
      this.loading = true;
      this.$api
        .get(`campaigns/${this.campaign.id}/links`)
        .then(resp => {
