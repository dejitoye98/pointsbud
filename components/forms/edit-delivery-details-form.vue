        ...
        <div class="form" @click.stop>
          <div class="form__container">
            <div class="form-input" v-for="(feature, index) in local_details" :key="index">
              <input
                type="text"
                placeholder="Name (e.g delivery medium)"
                style="margin-bottom:5px;"
                v-model="local_details[index].name"
              />
              <input
                type="text"
                placeholder="Value (e.g email address)"
                v-model="local_details[index].value"
              />
            </div>
    ...
    edit() {
      ...
      this.$api
        .put("/products/" + this.product.id, {
          delivery_details: this.local_details
        })
        .then(resp => {
          this.loading = false;
