    ...
    <div class="grid">
        ...
        <div class="grid__item">
            ...
            <ClicksReferrersTable
                v-if="clicks.top_referrers && clicks.top_referrers.data && clicks.top_referrers.data && clicks.top_referrers.data.length > 0"
                :clicks="clicks"
            />
        </div>
...
export default {
    ...
    methods: {
        ...
        initialize() {
            ...
            let url = `/insights/clicks?include=group_by_day,top_referrers&time=${this.clicks.volume.filter.by}`
            if (this.campaign) url += `&campaign_id=${this.campaign.id}`
            this.$api.get(url).then(resp=>{
