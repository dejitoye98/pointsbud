        ...
        <div>
            ...
            <div class="traffic">
                                            ...
                                            <div class="form-input">
                                                ...
                                                <div class="flex-row flex-row--w-50">
                                                    <label class="radio-label">
                                                        <input @change="changeProductHosting(1)" :value="parseInt('0')" name="who_can_join" type="radio" :checked="campaign.products.hosted">
                                                        <div class="radio-indicator"></div>
                                                        <div class="radio-text">Yes</div>

                                                    </label>
                                                    <label class="radio-label">
                                                        <input  @change="changeProductHosting(0)" :value="parseInt('1')" name="who_can_join" type="radio"  :checked="!campaign.products.hosted">
                                                        <div class="radio-indicator"></div>
                                        ...
                                        <div class="half">
                                            ...
                                            <div class="form-input form-input-file">
                                                ...
                                                <div class="file-container">
                                                    ...
                                                    <input @change="getFileName('display-picture')" type="file" id="display-picture"  accept="image/*" >
                                                </div>
                                            </div>
                ...
                <div class="steps__step">
                    ...
                    <div class="step-card">
                                ...
                                <div class="form-group advert-link"  v-for="(product,index) in campaign.products.list" :key="index">
                                    ...
                                    <div class="form-group form-group--row advert-link-row">
                                        ...
                                        <div class="form-input form-input-file">
                                            ...
                                            <button @click="showProductImagesModal(index)" @onImageUpdate="updateProductImages" :id="`product-${index}-images`"> Choose files </button>
                                        </div>
                                    ...
                                    <div v-if="campaign.products.list[index].meta.length > 0" class="advert-link form-group">
                                        ...
                                        <div class="form-input" v-for="(meta, meta_index) in campaign.products.list[index].meta" :key="meta_index">
                                                <input type="text" width="100%" :placeholder="'feature ' +parseInt( parseInt(meta_index) + 1)"   v-model="campaign.products.list[index].meta[meta_index]">
                                        </div>
                                    ...
                                    <div v-if="campaign.products.list[index].delivery_details.length > 0" class="advert-link form-group">
                                        ...
                                        <div class="form-input" v-for="(meta, meta_index) in campaign.products.list[index].delivery_details" :key="meta_index">
                                            <div class="half"> 
                                                <input type="text" style="display: block; width: 49%" width="100%" :placeholder="meta.name_placeholder"  v-model="meta.name">
                                                <input type=""  style="display: block; ; width: 49%" width="100%" :placeholder="meta.value_placeholder" v-model="meta.value">
                                            </div>
                                        </div>
                        ...
                        <div class="step-card">
                            ...
                            <div class="form">
                                <div class="form__container">
                                    <div class="form-input" v-for="(field, field_index) in campaign.order_form" :key="field_index">
                                        ...
                                        <template v-if="!field.changeable">
                                            ...
                                            <select disabled>
                                                ...
                                                <option v-for="(type, type_index) in ['text', 'number', 'date', 'one choice question', 'multi-choice question'].filter(a=> a!==field.field_type)" :key="type_index"> {{type}} </option>
                                            </select>
                                        </template>


                                        <template v-else>
                                            <input placeholder="Field Name (e.g Age)" style="margin-bottom: 5px;" type="text" v-model="campaign.order_form[field_index].field_name">
                                            <select v-model="campaign.order_form[field_index].field_type" style="margin-bottom: 5px;">
                                                <option v-for="(type, type_index) in ['text', 'number', 'date', 'one-choice option', 'multi-choice options']" :key="type_index"> {{type}} </option>
                                                
                                            </select>
                                            <!--- options --->
                                            <template v-if="['one-choice option', 'multi-choice option'].includes(campaign.order_form[field_index].field_type)">
                                                <input  v-model="campaign.order_form[field_index].field_options[option_index]" style="margin-bottom: 5px;" type="text"  :placeholder="'option '  + (parseInt(option_index) + parseInt(1))" v-for="(option , option_index) in campaign.order_form[field_index].field_options" :key="option_index"/> 
                            ...
                            <div class="form">
                                <div class="form__container">
                                    ...
                                    <div class="application_questions" v-if="[2, '2'].includes(campaign.who_can_join)">
                                        <div v-for="(q, q_index) in campaign.application_questions" :key="index" class="form-input">
                                            <span class="error" v-if="index === 0">{{errors.application_questions}}</span>
                                            <label for="">Application Question {{q_index + 1}} <span class="red" v-if="index === 0">*</span></label>
                                            ...
                                            <select>
                                                <option v-for="(type, q_index) in ['text', 'number', 'date', 'one-choice option', 'multi-choice options']" :key="q_index"> {{q_index}} </option>
                ...
                <template v-if="show_product_images_modal">
                    <SelectProductImagesModal @onImageUpdated="updateProductImages"  @onImagesSelected='addProductImages' :product_index="product_index_in_view" v-if="show_product_images_modal" @close="closeProductImagesModal"/>
                </template>
        </div>
</template>
...
export default {
    ...
    methods: {
        ...
        getFileName(element) {
            ...
            if (el) {
                el = el.value;
                var startIndex = (el.indexOf('\\') >= 0 ? el.lastIndexOf('\\') : el.lastIndexOf('/'));
        ...
        addProductDeliveryDetails(index) {
            this.campaign.products.list[index].delivery_details.push({name: '', name_placeholder: "Detail",  value: '', value_placeholder: 'Value'})
        ...
        addQuestion() {
            this.campaign.application_questions.push({question: '', answer_type: ''})
        },
        ...
        uploadProductImage(file, product_indext, image_index) {
            ...
            try {
                ...
                return xhr.onreadystatechange = function(e) {
                    ...
                    vm.campaign.products.list[product_index].images[image_index].loading = false
...
.steps {
    ...
    &__step {
            ...
            border-radius: 5px;
            width: 100%;
            border: 5px solid rgba(211, 211, 211, 0.442);
